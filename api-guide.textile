...
We use RequireJS for 'modularization' of javascript (and CSS) code, when built for production  mode the modules should mostly be in-lined into a central JS file(s) to avoid network latency issues.
...
Our WBT 'class' must be passed two dependiecies - Session (used to store the session state and connect to any LMS apis) and a Loader (which is responible for actually loading the site/wbt definition, parsing it, and initiating a callback to the WBT when done).
...
@prop    ds.s  Int The stragtegy to use when updating the datastore.  Defaults to 1.
...
@prop    lms.d    Delay loading objectives data from LMS API.  If set to 1 (not implemented) then delayed loading should take place.
...
The main concern is that these type of data stores operate on what is essentially a Javascript object/hash where the keys are defined dynamically by whatever code creates them, instead of statically.  This means we can not rely on a class / interface definition to document what values are available.
...
Retrive via:  store.locale  or  store['locale']
...
See the st definition.  This is a 'state' object representing the Topic state for the current user.  The 'state' information is derived from the SCORM 1.2 API, but where possible stored as a numberic index instead of a string value to save space when encoding (the data store is typically encoded and stored as a JSON string).
